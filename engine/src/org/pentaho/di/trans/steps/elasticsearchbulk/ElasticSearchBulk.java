  ...
  private void addSourceFromRowFields( IndexRequestBuilder requestBuilder, RowMetaInterface rowMeta, Object[] row )
    ...
    for ( int i = 0; i < rowMeta.size(); i++ ) {
      ...
      if ( StringUtils.isNotBlank( name ) ) {
        if(rowMeta.getValueMeta(i).isString())         { jsonBuilder.field(name, rowMeta.getValueMeta(i).getString(value));    }
        else if(rowMeta.getValueMeta(i).isDate())      { jsonBuilder.field(name, rowMeta.getValueMeta(i).getDate(value));      }
        else if(rowMeta.getValueMeta(i).isBoolean())   { jsonBuilder.field(name, rowMeta.getValueMeta(i).getBoolean(value));   }
        else if(rowMeta.getValueMeta(i).isNumber())    { jsonBuilder.field(name, rowMeta.getValueMeta(i).getNumber(value));    }
        else if(rowMeta.getValueMeta(i).isBigNumber()) { jsonBuilder.field(name, rowMeta.getValueMeta(i).getBigNumber(value)); }
        else { jsonBuilder.field(name, value); }
      }
    }
